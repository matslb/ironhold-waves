...
- Flat zones: `registerExplorationFlatZone(x, z, radius, blend, height, strength)` entries in `game.exploration.terrainFlatZones` lerp the sampler toward a target height. CONVENTION (T-025): ALL flat zones are registered BEFORE the ground mesh and biome patches bake. Presets come from `setupExplorationFlatZones()`; lake and village zones are registered up front in `setupExplorationWorld` from precomputed `lakeSpecs`/`villageSpecs` (seeded positions drawn once, then passed to `addExplorationLake`/`addExplorationVillage`, which no longer register zones themselves). Do NOT call `registerExplorationFlatZone` after the ground mesh bakes — a post-bake zone shifts the walk-height sampler away from the rendered mesh and characters sink/float (this was the historical villages/lakes bug, fixed the same way Crownford/Crownring were). If a new structure needs a flat zone, add its spec to that pre-bake block.
- Ground overlays (biome patches, roads, lake/bog water, pavement) must sample ground per-vertex or per-piece AFTER all flat zones are final, with a small lift above ground so they never z-fight: `BIOME_PATCH_LIFT` 0.032, `LAKE_SURFACE_LIFT` 0.084, `ROAD_SURFACE_LIFT` 0.112 (plus render-order constants next to them). Biome patches use `explorationRenderedGroundLocalY` (bilinear over the baked grid) so they hug the rendered mesh exactly; new wide overlays should do the same. Residual mesh-vs-sampler error away from any zone is just grid resolution (cell ~5.8u; typically <0.25, worst ~0.9 on steep lake-rim/blend-seam cells).
